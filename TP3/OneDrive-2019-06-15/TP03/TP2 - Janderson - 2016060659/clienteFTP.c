...
int main(int argc, char* argv[]){
	...
	char buffer_aux[strlen(argv[3])+21];
	...
	if(TAM_BUFFER-20 < strlen(nomeArquivo)){
		sprintf(buffer_aux, "%d##%d##", SEQ_S, ACK_S);
		strcat(buffer_aux, nomeArquivo);

		// Envio para o servidor do nome do arquivo a ser lido
		if(tp_sendto(clienteSocket, buffer_aux, strlen(buffer_aux), sockEndereco) <= 0){
			logExit("SEND");
		}

		gettimeofday(&tempoInicial, NULL); // obtém o tempo de início da comunicação com o servidor
	}
	else{
		strcpy(buffer, " "); // limpar o buffer
		sprintf(cabecalho, "%d##%d##", SEQ_S, ACK_S);
		strcpy(buffer, cabecalho);
		strcat(buffer, nomeArquivo);

		// Envio para o servidor o nome do arquivo a ser lido
	...
	while(1){
		...
		if(select(clienteSocket+1, &readySock, NULL, NULL, &timeOut) < 0 ){
			...
			nBytes = tp_recvfrom(clienteSocket, buffer, TAM_BUFFER, sockEndereco);
